...
public class DBTicket {
  ...
  public static int countTickets(Connection con, int userId, int eventId) {
    try (PreparedStatement stmt = con.prepareStatement(
        "SELECT COUNT(ticket_id) AS tickets FROM tickets WHERE userid = ? and event_id = ?")){
   ...
   * @return
   */
  public static boolean buyTickets(Connection con, int userId, int eventId, int eventUserId, int numTickets) {
    try (PreparedStatement stmt = con.prepareStatement(
        "UPDATE tickets SET userid = ? WHERE event_id = ? and userid = ? LIMIT ?")){
      stmt.setInt(1, userId);
      stmt.setInt(2, eventId);
      stmt.setInt(3, eventUserId);
  ...
  public static int getTicketAvail(Connection con, int eventId) throws SQLException{
    try (PreparedStatement stmt = con.prepareStatement(
      ...
      logger.info("Number of Tickets available: " + ticketCount);
      return ticketCount;
    } catch (SQLException e) {
      logger.error(e.getMessage());
      return -1;
  ...
  public static int getTicketPurchased(Connection con, int eventId) throws SQLException {
    try (PreparedStatement stmt = con.prepareStatement(
        "SELECT num_ticket_purchased AS tickets FROM events WHERE event_id = ?")){
      stmt.setInt(1, eventId);
      ResultSet result = stmt.executeQuery();
      int ticketCount;
      if (!result.next()) {
